...
import './women.css'; // Assuming you're using the same CSS
...
const Men = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const productCollectionRef = collection(db, 'men');
  ...
  const handleWhatsApp = (product) => {
    const productLink = `${window.location.origin}/#/men?id=${product.id}`; // Change here
  ...
  const openProductPage = (product) => {
    setModalProduct(product);
    navigate(`#/men?id=${product.id}`); // Change here
  };
  ...
  return (
    <section id="shop-section" className="new-shop-section">
      <h2 className="cormorant-garamond-regular">Men</h2>
